...
function showForm(id) {
    ...
    sessionStorage.setItem('serviceType', service.name);
    updatePrice();
...
function setMode(mode) {
    currentMode = mode;
    sessionStorage.setItem('urgencyMode', mode === 'urgent' ? 'مستعجل ⚡' : 'عادي 🕐');
...
function updatePrice() {
    ...
    sessionStorage.setItem('servicePrice', Number(price).toLocaleString() + ' SYP');
}
...
if ((regNo !== '' && !regex.test(regNo)) || (propNo !== '' && !regex.test(propNo))) {
    ...
    sessionStorage.setItem('customerMother',     mother);
    sessionStorage.setItem('customerNationalId', nationalId);
    sessionStorage.setItem('customerRegNo',      regNo);
    sessionStorage.setItem('customerPropNo',     propNo);
    sessionStorage.setItem('customerLocation',   loc);
